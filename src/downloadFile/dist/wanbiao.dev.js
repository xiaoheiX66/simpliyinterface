...
superagent.get('http://search.jumei.com/').query({
  ...
  $('.products_wrap li').each(function (i, el) {
    ...
    var goods = {
      ...
      slName: $el.find('.s_l_name a').text().replace(/\s*/g, ""),
  ...
  goodsList = goodsList.map(function (item) {
    // console.log("itemimg",typeof(item.slpicImg));
    if (typeof item.slpicImg === 'undefined') {
      item.shopType = '【非自营】';
      item.slpicImg = 'http:' + '//p4.jmstatic.com/product/005/304/5304564_std/5304564_350_350.jpg';
      item.slName = '超值!,圣罗兰 经典气垫包';
      item.newPrice = '125';
      item.oldPrice = '¥299';
      item.searchDeal = '距离结束还剩3天';
      item.listButton = '去看看';
    }
    ...
    insertSql.push("('".concat(item.shopType, "','").concat(filename, "','").concat(item.slName, "','").concat(item.newPrice, "','").concat(item.oldPrice, "','").concat(item.searchDeal, "','").concat(item.listButton, "')"));
    return item;
  });
  console.log("新的goodList", goodsList); // 写入数据库
  // insertSql=['()','()']

  var sql = 'insert into wanbiao(name,price,oldprice,imgurl) values' + insertSql.join(','); // console.log('sql',sql);
